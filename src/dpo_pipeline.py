...
def process_single_user(
    ...
    print(f"[User {uid}] Step 5: 评分候选画像（并发 workers={workers}）...", flush=True)
    ...
    if eff_workers == 1:
        for i, cand in enumerate(candidates):
            ...
            print(f"  候选 {idx+1}: Q_W0={c0:.4f} Q_W1={c1:.4f} Q_W2={c2:.4f} r_all={r_all:+.4f}", flush=True)
    else:
    ...
    dpo_pairs = construct_dpo_pairs(s0, candidates, s0_scores, candidate_scores_list)
...
def run_dpo_pipeline(
    ...
    print(f"[Pipeline] 处理 {len(users)} 个用户（本 run），候选并发 workers={workers}", flush=True)
...
if __name__ == "__main__":
    ...
    run_dpo_pipeline(args.input, args.output_dir, args.max_users, args.seed, args.workers)

...
Generate real user profiles using LLM, replacing simple UserID placeholders
"""
import json
import random
import psycopg2
...
STEP = 1
TEXT_LONG = 500
# Maximum actions to use for profile generation (N recent actions for FREE_FORM_PROMPT)
MAX_ACTIONS_FOR_PROFILE = 100
# Maximum actions to use for SFT sample construction (N recent actions per user)
...
HEARTBEAT_INTERVAL = 15   # Progress heartbeat interval (seconds), print progress
...
def format_actions_for_profile(actions, max_len=MAX_ACTIONS_FOR_PROFILE):
    ...
    # Limit to max_len most recent actions
...
def _stream_user_actions(community_id, selected_user_ids=None):
    ...
    """
    conn = get_db_connection()
    # Use psycopg2 server-side cursor with itersize for streaming
...
def generate_sft_data(community_id, output_file, use_real_profile=True, max_users=None):
    ...
    # If max_users specified, randomly sample user IDs
